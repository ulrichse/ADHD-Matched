...
matched_df <- read_parquet("matched_df_ADHD_annual_lag7.parquet")
matched_cb <- matched_cb_lag7(data)

combined_data <- get_daily_lags(matched_df, matched_cb)
write.csv(combined_data, "Results/daily_lags_annual.csv")
...
write.csv(merged_df, "Results/Annual_Cumulative_Lag7.csv")
gc()

matched_df <- matched_df_lag5(data)
write_parquet(matched_df, "matched_df_lag5_annual.parquet")
matched_cb <- matched_cb_lag5(data)

merged_df <- cumulative_lag0_lag5(matched_df, matched_cb)
merged_df <- merged_df %>%
  dplyr::select(outcome, over_rr, over_rr_se, over_rr_low, over_rr_high, estmean) 
write.csv(merged_df, "Results/Annual_Cumulative_Lag5.csv")
gc()

matched_df <- matched_df_lag3(data)
write_parquet(matched_df, "matched_df_lag3_annual.parquet")
matched_cb <- matched_cb_lag3(data)

merged_df <- cumulative_lag0_lag3(matched_df, matched_cb)
merged_df <- merged_df %>%
  dplyr::select(outcome, over_rr, over_rr_se, over_rr_low, over_rr_high, estmean) 
write.csv(merged_df, "Results/Annual_Cumulative_Lag3.csv")
gc()
